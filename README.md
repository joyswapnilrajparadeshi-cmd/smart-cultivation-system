🌱 Smart Cultivation System

[![Live Demo](https://img.shields.io/badge/Live-Demo-green?style=for-the-badge&logo=google-chrome&logoColor=white)](https://smart-cultivation.kesug.com)




<img width="1883" height="758" alt="image" src="https://github.com/user-attachments/assets/593afbe2-e835-441e-ab2c-5b3a4adf925f" />






A complete smart agriculture management system to help farmers, admins, and experts manage crops, track growth stages, and automate notifications.

Deployed here: smart-cultivation.kesug.com

🚀 Features
Farmer Dashboard

🌾 View crops and stages

📝 Submit reports and updates

📧 Receive notifications from admins

Admin Dashboard

➕ Add, edit, delete crops

👤 Manage farmer profiles

📊 Update crop stages and send notifications

📚 Knowledge base management

PHPMailer Integration

Automatic email notifications to farmers

🛠 Tech Stack



## 🛠 Tech Stack

![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![PHPMailer](https://img.shields.io/badge/PHPMailer-ff69b4?style=for-the-badge)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![XAMPP](https://img.shields.io/badge/XAMPP-ff8c00?style=for-the-badge)
![InfinityFree](https://img.shields.io/badge/InfinityFree-00bfff?style=for-the-badge)






💻 Project Structure
smart-cultivation-system/
├─ PHPMailer-master/          # Email library
├─ admin_dashboard.php
├─ farmer_dashboard.php
├─ crop_management.php
├─ knowledge_base.php
├─ login.php
├─ register.php
├─ db_connection.php
├─ smart_cultivation.sql      # Database
├─ ...                        # Other PHP files
🎬 Demo / Screenshots


<img width="1878" height="847" alt="image" src="https://github.com/user-attachments/assets/bdfad94f-3cda-4ed8-be13-7c8a4357e6a5" />


⚡ Installation

Clone the repository:

git clone https://github.com/joyswapnilrajparadeshi-cmd/smart-cultivation-system.git

Import the database smart_cultivation.sql in phpMyAdmin.

Update db_connection.php with your database credentials.

Start XAMPP/WAMP/LAMP server and open:

http://localhost/smart-cultivation-system/


📬 Contact / Feedback

Email: joyswapnilrajparadeshi@gmail.com

GitHub: joyswapnilrajparadeshi-cmd

✅ License

This project is open-source and free for educational purposes.
