...
<div class="dashboard">
    <div class="sidebar">
        <h2>
            ...
            <?php echo $translations['admin_dashboard'] ?? 'Admin Dashboard'; ?>
        </h2>
        <nav>
            <a href="admin_dashboard.php">
                <i class="fas fa-home"></i>
                <span><?php echo $translations['home'] ?? 'Home'; ?></span>
            </a>
            <a href="crop_management.php">
                <i class="fas fa-seedling"></i>
                <span><?php echo $translations['crop_management'] ?? 'Crop Management'; ?></span>
            </a>
            <a href="knowledge_base_admin.php">
                <i class="fas fa-book"></i>
                <span><?php echo $translations['knowledge_base'] ?? 'Knowledge Base'; ?></span>
            </a>
            <a href="notifications_admin.php">
                <i class="fas fa-bell"></i>
                <span><?php echo $translations['notifications'] ?? 'Notifications'; ?></span>
            </a>
            <a href="farmers_admin.php">
                <i class="fas fa-users"></i>
                <span><?php echo $translations['farmer_management'] ?? 'Farmer Management'; ?></span>
            </a>
        </nav>
    ...
    <div class="main">
        <a href="admin_dashboard.php">
            <i class="fas fa-arrow-left"></i> <?php echo $translations['back_to_dashboard'] ?? 'Back to Dashboard'; ?>
        </a>
        ...
        <div class="header">
            ...
            <div>
                <button id="openAdd" class="action-btn btn-add">
                    <i class="fas fa-plus"></i> <?php echo $translations['add_crop'] ?? 'Add Crop'; ?>
                </button>
            </div>
        </div>
        ...
        <div class="table-wrap">
            <table>
                <thead>
                    <tr>
                        <th>#</th>
                        <th><?php echo $translations['farmer'] ?? 'Farmer'; ?></th>
                        <th><?php echo $translations['crop'] ?? 'Crop'; ?></th>
                        <th><?php echo $translations['variety'] ?? 'Variety'; ?></th>
                        <th><?php echo $translations['field_plot'] ?? 'Field / Plot'; ?></th>
                        <th><?php echo $translations['planting_date'] ?? 'Planting Date'; ?></th>
                        <th><?php echo $translations['growth_stage'] ?? 'Growth Stage'; ?></th>
                        <th><?php echo $translations['yield'] ?? 'Yield'; ?></th>
                        <th><?php echo $translations['last_updated'] ?? 'Last Updated'; ?></th>
                        <th class="small"><?php echo $translations['actions'] ?? 'Actions'; ?></th>
                    </tr>
                </thead>
                <tbody id="cropBody">
                <?php
                if($result && $result->num_rows>0){
                    ...
                    while($row = $result->fetch_assoc()){
                        ...
                        echo "<tr data-id='{$pid}'>
                                ...
                                <td>
                                    <button class='action-btn btn-update open-update' data-id='{$pid}'><span>".($translations['update'] ?? 'Update')."</span></button>
                                    <button class='action-btn btn-delete delete-crop' data-id='{$pid}'><span>".($translations['delete'] ?? 'Delete')."</span></button>
                                </td>
                              </tr>";
                        $i++;
                    }
                } else {
                    echo "<tr><td colspan='10' class='small'>".($translations['no_crops_found'] ?? 'No crops found.')."</td></tr>";
                }
                ?>
                </tbody>
            </table>
...
<div class="modal" id="updateModal">
    <div class="panel">
        ...
        <h3>
            ...
            <?php echo $translations['update_growth_stage'] ?? 'Update Growth Stage'; ?>
        </h3>
        ...
        <form id="updateForm">
            ...
            <label><?php echo $translations['new_growth_stage'] ?? 'New Growth Stage'; ?></label>
            <select name="growth_stage" id="upd_stage" class="input" required>
                <option value=""><?php echo $translations['select_stage'] ?? '-- Select Stage --'; ?></option>
                <?php foreach($stages as $s) echo "<option value=\"{$s}\">{$s}</option>"; ?>
            </select>
            <div style="margin-top:20px;text-align:right">
                <button type="submit" class="action-btn btn-update">
                    <i class="fas fa-save"></i> <?php echo $translations['save'] ?? 'Save'; ?>
                </button>
            </div>
        </form>
    </div>
...
<div class="modal" id="addModal">
    <div class="panel">
        ...
        <h3>
            ...
            <?php echo $translations['add_crop_admin'] ?? 'Add Crop (Admin)'; ?>
        </h3>
        <div id="addMsg"></div>
        <form id="addForm">
            <label><?php echo $translations['farmer'] ?? 'Farmer'; ?></label>
            <select name="user_id" class="input" required>
                <option value=""><?php echo $translations['select_farmer'] ?? '-- Select Farmer --'; ?></option>
                <?php foreach($farmers as $f) echo "<option value=\"{$f['id']}\">".htmlspecialchars($f['fullname'])."</option>"; ?>
            </select>
            <label><?php echo $translations['crop_name'] ?? 'Crop Name'; ?></label>
            <input type="text" name="crop_name" class="input" required />
            <label><?php echo $translations['variety'] ?? 'Variety'; ?></label>
            <input type="text" name="variety" class="input" />
            <label><?php echo $translations['field_plot'] ?? 'Field/Plot'; ?></label>
            <input type="text" name="field" class="input" />
            <div class="form-row">
                <div class="col">
                    <label><?php echo $translations['planting_date'] ?? 'Planting Date'; ?></label>
                    <input type="date" name="planting_date" class="input" />
                </div>
                <div class="col">
                    <label><?php echo $translations['growth_stage'] ?? 'Growth Stage'; ?></label>
                    <select name="growth_stage" class="input">
                        <?php foreach($stages as $s) echo "<option value=\"{$s}\">{$s}</option>"; ?>
                    </select>
                </div>
            </div>
            <label><?php echo $translations['yield'] ?? 'Yield'; ?></label>
            <input type="text" name="yield" class="input" />
            <label><?php echo $translations['notes'] ?? 'Notes'; ?></label>
...
document.querySelectorAll('.open-update').forEach(btn=>{
    btn.addEventListener('click', function(){
        ...
        const currentText = '<?php echo $translations['current'] ?? 'current'; ?>';
        document.getElementById('upd_id').value = id;
        document.getElementById('upd_cropinfo').textContent = `${crop} — ${farmer} (${currentText}: ${stage})`;
...
document.getElementById('updateForm').addEventListener('submit', async function(e){
    ...
    const selectStageText = '<?php echo $translations['select_stage'] ?? 'Select a stage'; ?>';
    const savingText = '<?php echo $translations['saving'] ?? 'Saving...'; ?>';
    const saveText = '<?php echo $translations['save'] ?? 'Save'; ?>';
    const stageUpdatedText = '<?php echo $translations['stage_updated'] ?? 'Stage updated & notification sent.'; ?>';
    const updateFailedText = '<?php echo $translations['update_failed'] ?? 'Update failed'; ?>';
    const networkErrorText = '<?php echo $translations['network_error'] ?? 'Network error'; ?>';
    
    if(!growth_stage) { alert(selectStageText); return; }
    const btn = this.querySelector('button[type="submit"]');
    btn.disabled = true; btn.textContent = savingText;
    try {
        const res = await ajaxPost('admin_update_stage.php', { id, growth_stage });
        if(res.status === 'success'){
            document.getElementById('updateMsg').innerHTML = '<div class="success">'+stageUpdatedText+'</div>';
            ...
            document.getElementById('updateMsg').innerHTML = '<div class="err">'+(res.message||updateFailedText)+'</div>';
        }
    } catch(e){
        document.getElementById('updateMsg').innerHTML = '<div class="err">'+networkErrorText+'</div>';
    } finally { btn.disabled = false; btn.textContent = saveText; }
});

// Delete crop
document.querySelectorAll('.delete-crop').forEach(btn=>{
    btn.addEventListener('click', async function(){
        const deleteConfirmText = '<?php echo $translations['delete_crop_confirmation'] ?? 'Delete this crop? This cannot be undone.'; ?>';
        const deleteFailedText = '<?php echo $translations['delete_failed'] ?? 'Delete failed'; ?>';
        const networkErrorText = '<?php echo $translations['network_error'] ?? 'Network error'; ?>';
        ...
        try {
            ...
            if(res.status === 'success'){
                ...
                alert(res.message || deleteFailedText);
            }
        } catch(e){
...
document.getElementById('addForm').addEventListener('submit', async function(e){
    ...
    const btn = this.querySelector('button[type="submit"]');
    const addingText = '<?php echo $translations['adding'] ?? 'Adding...'; ?>';
    const addCropText = '<?php echo $translations['add_crop'] ?? 'Add Crop'; ?>';
    const cropAddedText = '<?php echo $translations['crop_added_success'] ?? 'Crop added successfully.'; ?>';
    const addFailedText = '<?php echo $translations['add_failed'] ?? 'Failed to add'; ?>';
    const networkErrorText = '<?php echo $translations['network_error'] ?? 'Network error'; ?>';
    
    btn.disabled = true; btn.textContent = addingText;
    document.getElementById('addMsg').innerHTML = '';
    try {
        const res = await ajaxPost('admin_add_crop.php', body);
        if(res.status === 'success'){
            document.getElementById('addMsg').innerHTML = '<div class="success">'+cropAddedText+'</div>';
            setTimeout(()=> location.reload(), 800);
        } else {
            document.getElementById('addMsg').innerHTML = '<div class="err">'+(res.message||addFailedText)+'</div>';
        }
    } catch(e){
        document.getElementById('addMsg').innerHTML = '<div class="err">'+networkErrorText+'</div>';
    } finally { btn.disabled = false; btn.textContent = addCropText; }
});
</script>
