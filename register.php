...
function sendOTPEmail($email, $otp) {
    ...
    try {
        ...
        $mail->Password   = 'kdbddmffsotggrjt'; // App Password
        $mail->SMTPSecure = 'tls';
        $mail->Port       = 587;
        $mail->SMTPOptions = [
            'ssl' => [
        ...
        $mail->Body    = "
            <h3>Smart Cultivation System</h3>
            <p>Your OTP is:</p>
        ...
        error_log("PHPMailer Error in registration: " . $mail->ErrorInfo);
...
if (isset($_POST['form_step']) && $_POST['form_step'] === '1') {
    ...
    foreach (['fullname','username','email','mobile','age','gender'] as $f) {
        if (empty($_POST[$f])) {
    ...
    $_SESSION['reg_data'] = array_merge($_SESSION['reg_data'], [
...
if (isset($_POST['form_step']) && $_POST['form_step'] === '2') {
    ...
    foreach (['aadhar','caste','total_land','role','location_name'] as $f) {
        if (empty($_POST[$f])) {
    ...
    $_SESSION['reg_data'] = array_merge($_SESSION['reg_data'], [
        ...
        'latitude'   => $_POST['latitude'] ?? '17.3850',
        'longitude'  => $_POST['longitude'] ?? '78.4867',
...
if (isset($_POST['form_step']) && $_POST['form_step'] === '3') {
    ...
    if (empty($_SESSION['reg_data']['email'])) {
    ...
    if ($sent === true) {
        ...
        $message = "OTP sending failed: " . ($sent ? $sent : "Unknown error");
        error_log("OTP sending failed for registration: " . ($sent ? $sent : "Unknown error") . " | Email: " . $_SESSION['reg_data']['email']);
        $showForm = true; // Keep form visible so user can see error and try again
...
if (isset($_POST['form_step']) && $_POST['form_step'] === 'verify_otp') {
    ...
    if ($_POST['otp'] == $_SESSION['otp'] && time() - $_SESSION['otp_time'] <= 300) {
        ...
        $stmt = $conn->prepare(
            "INSERT INTO users 
            (fullname, username, email, password, role, mobile, district, age, gender, aadhar, caste, total_land, latitude, longitude, location_address, created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())"
        );
        ...
        $stmt->bind_param(
            ...
            $d['mobile'], $d['district'], $d['age'], $d['gender'], $d['aadhar'],
            $d['caste'], $d['total_land'], $d['latitude'], $d['longitude'], $d['location']
        );
